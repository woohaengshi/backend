...
public class PasswordService {
    ...
    private MimeMessage createMail(Member member) {
        ...
        String reissueLink = "https://woohangshi.vercel.app/password/" + authenticationCode.getCode();
        MimeMessage mimeMessage = mailSender.createMimeMessage();
        try {
            mimeMessage.setFrom(WOOHAENGSHI_MAIL);
            mimeMessage.setSubject("[우행시] 비밀번호 재설정을 위한 임시 번호 발급 안내");
            mimeMessage.setText(String.format(CONTENT_FORMAT, member.getName(), authenticationCode.getCode(), reissueLink));
    ...
    private Member findMemberById(Long memberId) {
        return memberRepository.findById(memberId).orElseThrow(() -> new WoohaengshiException(MEMBER_NOT_FOUND));
    }
